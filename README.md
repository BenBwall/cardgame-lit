...
Click a card to play it. When matching ranks are available, clicking one starts selection: choose the cards you want and press **Play selected**, or **Cancel** to clear the selection without playing. **Select all** selects an entire matching set but still waits for **Play selected**. **Select matching cards** and Shift-click also start selection. This works for both hand and face-up cards. Hover or focus the play pile or Out (exile) pile to see every card, top card first. Previews scroll for large piles and close with Escape. Click the central pile to pick it up. During setup, choose one hand card and then a face-up table card to swap. The hand uses the same fan/grid layouts, suit/rank/draw/manual sorting, mouse/touch/pen dragging, Alt + arrow/Home/End reordering, and draw-flip controls as free play. Hand arrangement never consumes a turn. Face-down cards stay beneath their covers in three fixed table positions; the computer's hand is shown as backs.
