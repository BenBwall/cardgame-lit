...
A Lit card table based on [BenBwall/cardgame](https://github.com/BenBwall/cardgame), originally ported from commit `adceeca406f95b9044913ff2300e4484364e14a5`.

Choose the **Free play** or **Shithead** tab. Shithead has **Single player** (against the computer) and **Multiplayer** subtabs. Arrow keys, Home and End switch tabs; the last selected Shithead mode is remembered. Free play preserves the original shuffled 52-card deck, drawing, playing, ace-low sorting, undo, and hand arrangement. Both local games and their settings are saved automatically in browser storage and restored after a refresh or return visit. Switching tabs keeps an online room connected.

Online Shithead uses a separately hosted authoritative Bun server. Choose any display name, create a room with your rules, or join using a six-character case-insensitive code. Duplicate display names are allowed; random room-scoped credentials identify players. Both humans choose setup swaps and mark themselves ready. The server never sends opponents' hidden cards. Connections recover automatically while the tab stays open; reloading loses the anonymous session because credentials are kept only in memory. Leaving closes the room. Finished games support unanimous rematches. [Server setup, protocol, security, extension guide and tests](server/README.md).

## Shithead

Click the vertical **Card appearance** tab on the left edge to choose faces and backs independently. Settings open in a scrollable popout without moving the table; close it with the tab, the close button, Escape, or a click outside. New players start with **GreyWyvern · Wildlife** faces and **GreyWyvern · Random per card** backs. Existing saved artwork choices are preserved. Two complete CC0 decks (Kenney Pixel and GreyWyvern Wildlife) and 14 alternative backs are built in alongside **Basic (HTML+CSS)**, formerly Original. Basic backs have a main color picker and diagonal, vertical, horizontal, or plain (no stripes) patterns, shared by the preview and game cards and saved in this browser. The optional secondary stripe color automatically follows the main color by moving HSL lightness 15% toward white until you pick a custom color. Use **Use automatic stripe color** to clear the override; custom colors and automatic mode persist across reloads. Random backs assign a back to each card once per game. The assignments survive moves, undo, mode changes, and reloads; a new deal assigns new backs. Individual fixed GreyWyvern backs remain available. Kenney sprites display without their transparent outer padding, keeping the visible card at the normal proportions. Source links and original license notices are in [CARD-ART-LICENSES.md](CARD-ART-LICENSES.md).

Upload a PNG, JPEG, or WebP for a back or a chosen card, or import several faces named like `A-Spades.png`, `10-Hearts.jpg`, and `K-Clubs.webp`. A partial custom deck uses basic faces for missing cards. Files stay in the browser, are fitted without cropping to 240 × 348 pixels, and are saved separately from game state. Individual uploads are limited to 5 MB / 16 megapixels, with a 2 MB total encoded artwork limit. Invalid batches leave the previous artwork intact. If storage is full or unavailable, a notice appears and the previous saved artwork stays selected. Clear controls remove custom images. Artwork follows cards through drag/flip animations and pile previews, and applies to both modes without restarting either game.
...
Consumers bundle `dist/index.js` with its `lit` peer dependency, or provide an import map for `lit`. A TypeScript host can import `src/index.ts` directly and compile the source. Source imports use `@cardgame/*` and server imports use `@server/*`, defined in `tsconfig.paths.json`. Configure these aliases when consuming source. The package build rewrites aliases to relative imports in both JavaScript and declarations. When bundling, copy `dist/assets/` beside the output module (or configure your bundler to preserve the module-relative image URLs). A direct ES-module host should keep `dist/` intact. No CDN is needed. Each component has independent state. `CardArtwork` uses named face/back style unions, card-keyed custom images, and validated `HslColor` values. `readArtwork` migrates legacy `original` styles to `basic` and legacy hex colors to HSL without discarding custom images. Create the element in the browser; no SSR or hydration is required for the game.
